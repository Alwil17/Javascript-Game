...
function playUpdate() {
  ...
  for (var i = 0; i < redNum; i++) {
    ...
    if (goodArc.state[i]) {
      if (
        player.x < goodArc.x[i] + rad &&
        player.x + 30 + rad > goodArc.x[i] &&
        player.y < goodArc.y[i] + rad &&
        player.y + 30 > goodArc.y[i]
      ) {
        score++;
  ...
  for (var i = 0; i < blackNum; i++) {
    if (
      player.x < badArc.x[i] + rad &&
      player.x + 30 + rad > badArc.x[i] &&
      player.y < badArc.y[i] + rad &&
      player.y + 30 > badArc.y[i]
    ) {
      lives--;
